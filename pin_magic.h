...
#define USE_ADAFRUIT_SHIELD_PINOUT
  ...
  #define RD_MASK B00010000 // TODO: verify the MSB vs LSB
  #define WR_MASK B00100000
  #define CD_MASK B01000000
  #define CS_MASK B10000000
  ...
  #define setReadDirInline()  { DDRC &= ~B11111111; }
  #define RESET_DISABLE { RESET_PORT |= RESET_MASK; }
  #define RESET_ENABLE  { RESET_PORT &= ~RESET_MASK; }
  // quick helper (port, mask)
  #define SET_OUTPUT(x, y) { (x) |= (y); }
  #define SET_INPUT(x, y) { (x) &= ~(y); }
  #define SET_OUTPUTS {         \
    SET_OUTPUT(DDRD, 0b11111000);} 
    /*SET_OUTPUT(DDRD, RD_MASK); \
    SET_OUTPUT(DDRD, WR_MASK); \
    SET_OUTPUT(DDRD, CD_MASK); \
    SET_OUTPUT(DDRD, CS_MASK); \
    SET_OUTPUT(DDRD, RESET_MASK); \*/
 #else
  #error "ATmega32 not configured correctly"
 #endif
